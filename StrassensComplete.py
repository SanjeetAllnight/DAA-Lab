...
def main():
    try:
        ...
        while size<n:
            size*=2
        A1=[r+[0]*(size-n) for r in A]+[[0]*size for _ in range(size-n)]
        B1=[r+[0]*(size-n) for r in B]+[[0]*size for _ in range(size-n)]
        start=time.perf_counter()
        C1=standard(A,B)
        t1=time.perf_counter()-start
        start=time.perf_counter()
        C2=strassen(A1,B1,size)
        t2=time.perf_counter()-start
        C2=[r[:n] for r in C2[:n]]
        ...
        for row in C2:
            print(*row)
        print("Standard time:",t1)
        print("Strassen time:",t2)
